...
function validate_userName($userName){
    return strlen($userName)>10;
}

function validate_location($location){
    return strlen($location)>10;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($userName)){
        ...
        $userName_error=" must be at least more than or equal 16";
    }else{
        $userName_valid=true;
    };
    ...
    if(empty($password)){
        ...
        $password_error = "password must be at least more than or equal 16";
    }else{
        $password_valid=true;
    };
    ...
    if (empty($creditCard)){
        ...
        $creditCard_error= "Credit card must be at least more than or equal 16";
    }else{
...
require("../../views/register/customer.register.view.php");
